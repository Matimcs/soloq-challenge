...
async function runGrant({ q, KEY }){
  ...
  // Smurfs por usuario (para trackear también sus cuentas alternas).
  const smurfs = await q("SELECT user_id, riotid FROM smurfs WHERE riotid IS NOT NULL AND riotid <> ''");
  const smurfsByUser = {};
  for (const s of smurfs) (smurfsByUser[s.user_id] = smurfsByUser[s.user_id] || []).push(s.riotid);
  const accountsOf = u => [u.riotid, ...(smurfsByUser[u.id] || [])];

  // Mapa puuid -> userId de TODAS las cuentas (main + smurfs): así "robar shell al ganar a un
  // rival" funciona aunque el rival (o tú) estén jugando en una smurf.
  const puuidToUser = {};
  for (const u of users){
    for (const rid of accountsOf(u)){ const p = await getPuuid(rid, KEY); await sleep(80); if (p) puuidToUser[p] = u.id; }
  }
  ...
  for (const u of users){
    try {
      ...
      let prog = rows[0];
      // Progreso POR CUENTA: { riotid: {last_end, streak, champ_wins, castigo_wins} }.
      let acc = (prog && prog.accounts && typeof prog.accounts === 'object' && !Array.isArray(prog.accounts)) ? { ...prog.accounts } : {};
      // Migración del estado legacy (por-usuario) → cuenta main, una sola vez.
      if (prog && !acc[u.riotid] && Number(prog.last_end) > 0){
        acc[u.riotid] = { last_end: Number(prog.last_end) || 0, streak: Number(prog.streak) || 0,
          champ_wins: Array.isArray(prog.champ_wins) ? prog.champ_wins : [], castigo_wins: Number(prog.castigo_wins) || 0 };
      }
      if (!prog) await q("INSERT INTO shell_progress (user_id,accounts) VALUES ($1,'{}') ON CONFLICT (user_id) DO NOTHING", [u.id]);

      for (const rid of accountsOf(u)){
        const puuid = await getPuuid(rid, KEY); await sleep(60); if (!puuid) continue;
        let st = acc[rid];

        // Línea base por cuenta: la primera vez que vemos esta cuenta NO repartimos retroactivo.
        if (!st){
          ...
          acc[rid] = { last_end: base, streak: 0, champ_wins: [], castigo_wins: 0 };
          continue;
        }

        const lastEnd = Number(st.last_end) || 0;
        ...
        let champWins   = Array.isArray(st.champ_wins) ? st.champ_wins.slice() : [];
        let castigoWins = Number(st.castigo_wins) || 0;
        ...
        for (const id of ids.slice().reverse()){
          ...
          // ----- Racha de 6 victorias (por cuenta) -----
          ...
          if (win){
            ...
            for (const p of (m.info.participants || [])){
              ...
              if (!enemyId || enemyId === u.id) continue;        // rival registrado (no yo, ni mi otra cuenta)
        ...
        acc[rid] = { last_end: newLastEnd, streak, champ_wins: champWins, castigo_wins: castigoWins };
      }

      await q('UPDATE shell_progress SET accounts=$2::jsonb, updated_at=now() WHERE user_id=$1', [u.id, JSON.stringify(acc)]);
